...
QC_FREQ = 18000 # when to output full wavefield or not (I/O heavy)
n_processes = 42
...
ds = 4# downsample factor (for pre-processing)100000
####
...
## Place sources
if RANDOMSOURCE:# N sources per velocity model
    source_is = np.random.randint(4+NPOINTS_PML, NX-NPOINTS_PML-4, (N_VELS,N_SOURCES , 1))
    source_is = np.concatenate([source_is, SOURCE_Yi*np.ones((N_VELS, N_SOURCES, 1), dtype=int)], axis=2)
...
def add_noise(signal, snr):#添加噪声
 ...
 #     noise = acoustics.generator.white(signal.size).reshape(*signal.shape)
    noise = 2*(np.random.random(signal.shape)-0.5)
    # For the record I think np.random.random does exactly the same thing

    # work out the current SNR
    current_snr = 10*np.log10(np.linalg.norm(signal) / np.linalg.norm(noise))
    after = 10**(snr/10)/10**(current_snr/10)
...
def generate_example(ivel):
    ...
    for isource,source_i in enumerate(source_is[SIM_NUM]):
        ...
        if NOISE == True:
            gather_decimated = add_noise(gather_decimated,25)
        # SAVE
