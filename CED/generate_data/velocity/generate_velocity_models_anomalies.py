...
import sys
import matplotlib
if 'linux' in sys.platform.lower(): matplotlib.use('TkAgg')# use a non-interactive backend (ie plotting without windows)
...
from add_anomalies import add_anomalies
...
class Constants:
    ...
    def __init__(self):
        ...
        self.RUN = "anomalies_freedom4_100"#命名使用 自由度+速度模型个数
        ...
        self.vm_bed_thickness_mu_L, self.vm_bed_thickness_sigma_L = 150., 20. # bed thickness (m)  ! RECONSIDER THIS DISTRIBUTION
        ...
        ## ANOMALIES MODEL PARAMETERS
        self.freedom = 4 #模型自由度
        self.anomal_thickness_percent = (0.3,0.5)#异常体层厚占该层厚度的百分比范围
        self.anomal_width_percent = (0.3,0.5)#异常体宽度占模型总宽度的百分比范围
        self.anomal_vertical_limit = (0.1,0.4)#异常体左上角垂向坐标的限制范围
        self.anomal_horizon_limit = (0.1,0.4)#异常体左上角水平坐标的限制范围
        self.anomal_dv_percent = (0.1,0.3)#速度异常扰动的百分比
...
if __name__ == "__main__":
    ...
    shutil.copyfile('generate_velocity_models_anomalies.py', c.VEL_DIR + 'generate_velocity_models_%s.py'%(c.RUN))
  
    # GENERATE 1D MODELS
    
    np.random.seed(c.random_seed)# for reproducibility
    vm_s_0 = [generate_1D_random_velocity_trace(c) for _ in range(1)]
    print(len(vm_s_0))
    vm_s = [val for val in vm_s_0 for i in range(c.n_examples)]
    print(len(vm_s))

    # CONVERT TO 2D, ADD ANOMALIES (MULTIPROCESSING)
    ...
    def generate_examples(example_indices):
        ...
        for i in example_indices:
            ...
            ###########################
            #根据自由度添加矩形异常体
            np.random.seed(c.random_seed)# set seed (distributed!)
            velocity = add_anomalies(c, velocity,i,vm_s)
            ###########################       
    ...
    np.random.seed(123)##异常体的random seed 只控制异常体
