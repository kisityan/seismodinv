...
def plot_result(inputs_array, outputs_array, labels_array, sample_batch, ib=0,  isource=0,
    ...
    t_gain = np.arange(inputs_array.shape[-1], dtype=np.float32)**2.5
    t_gain = t_gain/np.median(t_gain)
    t_gain = t_gain.reshape((1,1,1,inputs_array.shape[-1]))# along NSTEPS
    
    plt.subplot2grid((1, 5), (0, 0), colspan=1)
    plt.imshow((t_gain*inputs_array)[ib,isource,:,:].T,
               aspect=aspect, cmap="Greys", vmin=-1, vmax=1)
    plt.colorbar()
    plt.title('input')
    
    plt.subplot2grid((1, 5), (0, 1), colspan=2)
    plt.imshow(outputs_array[ib,0,:,:].T, vmin=-1, vmax=1)
    plt.colorbar()
    plt.title('output')
    
    plt.subplot2grid((1, 5), (0, 3), colspan=2)
    plt.imshow(labels_array[ib,0,:,:].T, vmin=-1, vmax=1)
    plt.colorbar()
    plt.title('label')
...
class Trainer:
    ...
    def __init__(self, c):
        ...
        # clear directories
        ####如果接着上次训练需要将下一行语句注释，防止清空summary
        c.get_outdirs()
        ...
        #device = torch.device("cuda:%i"%(c.DEVICE) if False else "cpu")
        ...
        # writer.add_graph(model, torch.zeros((1,128,128)+c.VELOCITY_SHAPE))# write graph before placing on GPU
        print()
    ...
    def train(self):
        ...
        for ie in range(N_EPOCHS):  # loop over the dataset multiple times
            ...
            for ib in range(N_BATCHES):
                ...
                inputs = sample_batch["labels"]# expects list of inputs
                labels = sample_batch["inputs"]# expects list of labels
                inputs.extend(labels[1:2])
                # print(len(labels))
                del labels[1]
                # print('inputs%s'%len(inputs))
                ...
                if (i + 1) % c.SUMMARY_FREQ == 0:
                    ...
                    with torch.no_grad():# faster inference without tracking
                        ...
                        # output to screen
                        print('[epoch: %i/%i, batch: %i/%i i: %i] l1loss: %.4f l2loss: %.4f rate: %.1f elapsed: %.2f hr %s %s' % (
                               ie + 1,
                               N_EPOCHS,
                               ib + 1, 
                               N_BATCHES, 
...
if __name__ == "__main__":
    ...
    DEVICE = 0
    cs = [Constants(MODEL = models.AE_r_inverse_new,
                    RUN="anomalies_cae_3_inverse_1SOURCE_500bs",
